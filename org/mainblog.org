...
NOTE: CUSTOM_ID and PUBLISHED are always required.
...
[2025-11-24 Mon 12:37]

SearchGen

First draft [2025-11-24 Mon 13:05]

** Literate Compiler/Programming Tests with Org Mode and Emacs
[2025-11-03 Mon 09:53]

- RunTest test suite

Wrote first draft [2025-11-03 Mon 11:02]

** Abusing the C++ Type System---LCC's Instruction Selection Implementation
[2025-11-03 Mon 09:53]

- isel_patterns.hh

Wrote first draft [2025-11-03 Mon 20:01]

** Glint's Basic Grammar
[2025-11-03 Mon 09:53]
- once I flesh it out, go over the single expression, group expression, call expression stuff, and why it fundamentally is a different grammar than C or LISP or Rust.
- accompanying video possibly?

Wrote first draft [2025-11-03 Mon 12:49]

** Formatting Glint using Emacs and Rust
[2025-11-03 Mon 09:53]
- glint-ts-mode formatting implementation using tree sitter queries

** How I Use Org Mode (and Only Org Mode) to Write This Blog
[2025-11-24 Mon 12:37]

** My Very Own Emacs---How I Wrote a LISP-based Text Editor
[2025-11-03 Mon 09:53]
- LITE tree sitter and graphical implementation
- accompanying video possibly?

** Implementing Org mode in Glint
[2025-11-03 Mon 09:53]

I haven't actually done this, yet...

** Glint Tutorial Series
[2025-11-03 Mon 15:58]



* Begrudgingly Trying AI :blogpost:
:PROPERTIES:
:CUSTOM_ID: 2026aiexploration
:CREATED: [2026-05-19 Tue 12:20]
:PUBLISHED: [2026-05-19 Tue 12:20]
:MODIFIED: [2026-05-19 Tue 12:20]
:OG_DESCRIPTION: Lens_r's exploration into AI, using old hardware to run local models.
:POST_TAGS: AI, curiosity, programming
:END:
...
* How It's Made: Word Search Puzzles :blogpost:
:PROPERTIES:
:CUSTOM_ID: 2025wordsearchgen
:CREATED: [2025-11-24 Mon 12:38]
:PUBLISHED: [2025-11-24 Mon 12:39]
:MODIFIED: [2025-11-24 Mon 12:39]
:OG_IMAGE: wordsearch2025.webp
:OG_DESCRIPTION: A word search is something all of us did either as kids or at your most recent baby shower. But, how are they even produced in the first place?
:POST_TAGS: curiosity, programming
:END:
...
You can find the full source code [[https://github.com/LensPlaysGames/word-search-generator--emacs-lisp][on GitHub]].

* Abusing C++'s Type System---LCC's Instruction Selection Implementation :blogpost:
:PROPERTIES:
:CUSTOM_ID: 2025lccisel
:CREATED: [2025-11-03 Mon 19:01]
:PUBLISHED: [2025-11-03 Mon 19:49]
:MODIFIED: [2025-11-03 Mon 19:49]
:OG_IMAGE: /todo
:OG_DESCRIPTION: How does a compiler know what machine instructions correspond to a source language's constructs?
:POST_TAGS: programming, LCC, cpp
:END:
...
If you aren't familiar with the various stages of a compiler, or the general structure, I implore you to check out the [[https://github.com/LensPlaysGames/LensorCompilerCollection/blob/main/docs/lcc/compilation_overview.pdf][relevant LCC documentation]] (this isn't a bad start; everything in that directory should help put together a picture).
...
(Relevant: [[https://github.com/LensPlaysGames/LensorCompilerCollection/blob/ebc417ca535aec027739a1154e8ace3942930109/include/lcc/codegen/isel.hh][isel.hh]])

In C++11, we gained the ability to create "packs" of either function or template parameters (see [[https://en.cppreference.com/w/cpp/language/parameter_pack.html][cppreference]]). A "pack" is just an arbitrary list of parameters. To operate on the items in that list, there are ways to apply folds to the pack using different operators. With this, we can use LISP-like ways of defining a working =While= and =Foreach=
...
(Relevant: [[https://github.com/LensPlaysGames/LensorCompilerCollection/blob/ebc417ca535aec027739a1154e8ace3942930109/docs/lcc/isel.org][docs/lcc/isel.org]])
...
Let's look at a pattern for x86_64:
#+begin_src c++
using ret = Pattern<
...
This is just about the simplest a pattern can get. Patterns may also get more complex, like in the case of x86_64 not handling immediates in some places directly, and requiring an intermediate register. Or division, which, on x86_64, has /multiple/, /fixed/ output registers. Or both :^).
...
Then, each time you want to emit code for the x86_64 architecture, these patterns are used to construct the lMIR that is passed to register allocation, and then the code generation format backend (i.e. assembly or object file).
...
https://github.com/LensPlaysGames/LensorCompilerCollection/blob/ebc417ca535aec027739a1154e8ace3942930109/docs/lcc/isel.org

https://github.com/LensPlaysGames/LensorCompilerCollection/blob/ebc417ca535aec027739a1154e8ace3942930109/include/lcc/codegen/isel.hh

* Glint's Quirky Call Syntax                                       :blogpost:
:PROPERTIES:
:CUSTOM_ID: 2025glintcallgrammar
:CREATED: [2025-11-03 Mon 11:02]
:PUBLISHED: [2025-11-03 Mon 12:30]
:MODIFIED: [2025-11-03 Mon 12:30]
:OG_IMAGE: /todo
:OG_DESCRIPTION: Ambiguous? I think not.
:POST_TAGS: glint, programming, LCC
:END:
...
* Literate Tests with Org Mode and Emacs Lisp :blogpost:
:PROPERTIES:
:CUSTOM_ID: 2025literatetests
:CREATED: [2025-11-03 Mon 10:01]
:PUBLISHED: [2025-11-03 Mon 12:30]
:MODIFIED: [2025-11-03 Mon 12:30]
:OG_IMAGE: /todo
:OG_DESCRIPTION: Testing, testing... Attention, please!
:POST_TAGS: LCC, programming, emacs, lisp, org
:END:
...
Emacs is also very portable (just like Bash) ((holy heck, it even runs on my /phone/ now!!)), so I think anyone who wants to hack on LCC will be able to get a working Emacs on their system.
...
So, because the last expression is not of =int= type, Glint inserts =return 0;=.
...
#+begin_src org
  ,#+NAME: output
  ,#+begin_src
  (lambda (output) (string-empty-p output))
  ,#+end_src
#+end_src
...
You can read actual RunTest tests at =runtest/corpus/= within the [[https://github.com/LensPlaysGames/LensorCompilerCollection][LCC repository]]. This can be a good way to get a feel for a language LCC compiles, as there are lots of example programs combined with an assertion of exactly what they do.

Still interested? Go read =runtest/runtest.el= within the [[https://github.com/LensPlaysGames/LensorCompilerCollection][LCC repository]]; it showcases a lot of very LISPy things, like =gensym=, macros, asynchronous subprocesses, and more, while still being short and sweet.
...
* A Glint of Hope :notpost:
:PROPERTIES:
:CUSTOM_ID: aglintofhoep
:CREATED: [2024-07-26 Fri 02:18]
:PUBLISHED: [2024-07-26 Fri 02:18]
:MODIFIED: [2024-07-26 Fri 02:18]
:OG_IMAGE: 2024.07.26.GlintLineCount.png
:OG_DESCRIPTION: Seeing the Light Reflect out of the Darkness... Wait, Are Those Eyes?
:POST_TAGS: glint, programming
:END:
...
** Tendency Towards Distemper

I have this terrible habit of being just /slightly/ too ambitious such that I become increasingly frustrated with my smidgen of inability to achieve what I want to, how fast I want to. It makes me so frustrated and grows into rage, until
...
** Glint Line Count as of 2024.07.26: 10005

Line count on Glint language only using https://github.com/AlDanial/cloc.
#+begin_example
-------------------------------------------------------------------------------
...
Why are =enum= and =struct= highlighted as a keyword but =void= and =bool= are highlighted as types...?
...
:POST_TAGS: programming, cpu, assembly, hardware
:END:
