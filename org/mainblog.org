...
** TODO How I Manage My Finances From the Command Line
...
* How I Manage My Finances From the Command Line :WIPpost:
:PROPERTIES:
:CUSTOM_ID: financialcommandlinemanagement
:CREATED: [2023-07-12 Wed 11:23]
:PUBLISHED: [2023-07-12 Wed 11:23]
:MODIFIED: [2023-07-15 Sat 06:50]
...
While double-entry accounting /is/ a little bit to wrap your head around, it's also a really good way to think about transactions, so I think it's worth the effort.

But before we get into that, let's first just go over how Ledger works. Unlike most accounting software I've found, which use either a proprietary binary "project" format or CSV, Ledger uses a plain-text file that *you* maintain however you'd like to; the software *never* alters your ledger file. To add a new transaction, you open the file in a text editor and add an entry; it is quite a human-friendly format. Let's look at an example transaction.
...
While that may be slightly confusing exactly what's going on, I have a good way to break it down for you. The date is in year\slash{}month\slash{}day format, followed by the "payee"; think of the payee like the name you would write on a check, or who is recieving the money: in this case, Starbucks. The following lines, indented with at least one space, are the postings of the transaction. Each posting has an account and an amount, which must be separated by at least two spaces. The accounts are entirely up to you how you keep track of things, just know they are hierarchical; that is, the total of `Expenses` will include the total of `Expenses:Wants`. While you /are/ able to use any accounts you would like, there is no requirement, Ledger CLI recommends these five top level categories, and everything else you sort into these. I highly recommend this system as well, to keep your ledger reports, well, legible.
...
With these five categories, it makes it quite easy to describe most transactions relatively easily.

You probably noticed the posting on the very bottom has no amount; this is because we are taking advantage of Ledger's ability to automatically calculate the amount needed to make the transaction equal to zero. In double entry accounting, you keep track of both where money comes from /and/ where money goes, so every transaction should add up to zero; in this case, Ledger will calculate ~$-9.08~. If you want, you could always write this value explicitly, but I see no reason to.

When you get payed, the transaction you record in your ledger file may look something like this.
